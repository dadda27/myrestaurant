/*This page contains the scripts that control the top main menu behaviours when the screen is shrank
  (e.g, smartphone displays width), so the main menu cannot be shown horizontally as in wider screens. 
  E.g., trigger the slide menu to show from top to bottom when clicking the burger button */
  let click = false;
...
getBurgerBtn.addEventListener("click", function() {
  ...
  if (click === false) {
    ...
    /*Changes the appearance of the menu burger button from a common 3 lines to an "X"*/
      getBurgerBtn.classList.add("open");

      /*If the slide menu doesn't fit entirely within the height of the display, 
        it makes the slide menu scrollable instead of cutting it, 
        else it shows the entire menu*/
      if (getWindowHeight  <=  getMenusHeight) {
        ...
        for (let i = 0; i < menuItemsLenght; i++) {
          ...
          /*if the slide-from-top menu is opened (browser window is narrow)
            and then the browser window is enlarged to fit normal horizontal menu, 
            it hides the slide-from-top menu and makes it unscrollable*/
    ...
    /*Closes the slide-from-top menu: changes the "X" appearance of the burger button to normal 3 lines
      and hides the menu above the top of the page*/
    else {
        click = false;
 ...
 /*Changes the transparency of the menu background from transparent to opaque when scrolling*/
